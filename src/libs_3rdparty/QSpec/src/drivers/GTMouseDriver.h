...
public:
    ...
    static bool doubleClick(const QPoint &p);
    static bool clickCurPos(Qt::MouseButton = Qt::LeftButton); // move and click
    static bool doubleClickCurPos();

    // fails if the given button is not Qt::LeftButton, Qt::RightButton or Qt::MiddleButton
    // Linux: fails if there is an opening X display error
    static bool press(const QPoint &p, Qt::MouseButton = Qt::LeftButton);
    static bool release(const QPoint &p, Qt::MouseButton = Qt::LeftButton);
    static bool pressCurPos(Qt::MouseButton = Qt::LeftButton);
    static bool releaseCurPos(Qt::MouseButton = Qt::LeftButton);    //release method should check if this key has been already pressed. Do nothing otherwise
